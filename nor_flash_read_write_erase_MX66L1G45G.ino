/*
 * Erase Write Read (EWR) cycle for MX66 Macronixfamily of NOR Flash.
 * 
 * note: read unique ID
*/
...
#define DDRRQIOR4 0x0E // DDR Quad I/O Read - CR1V[1] = 1 or CR2V[3] = 1

//Program Flash Array
#define PP 0x02 // Page Program
#define PP4 0x12 //Page Program
#define QPP 0x38 //Quad Page Program
...
#define CE 0x5B //Chip Erase
//#define CE_ALT 0xC7 //Chip Erase (Alternate Instruction)

//Erase/Program Suspend/Resume
#define RE 0x60h //program resume
#define PS 0x61h // program suspend
#define ER 0x62h // erase resume instruction
#define ES 0x63h // erase suspend instruction 
...
void software_reset(){
  //TODO: Allow Serial11 input from device to trigger this function
  digitalWrite(cs, LOW);
  SPI.transfer(RSTEN);
  SPI.transfer(RST);
...
void clear_screen(){
  //Clear the Serial11 output screen
...
void read_status_until_done() {
  ...
  */
  ...
  do{
    ...
    //Serial1.print("Status 1: ");
    //Serial1.println(status1, BIN);
    Serial1.print("WIP: ");
    Serial1.println((status1 >> WIP) & 0x1, BIN);
    Serial1.print("WEL: ");
    Serial1.println((status1 >> WEL) & 0x1, BIN);
    //Serial1.print("Status 2: ");
    //Serial1.println(status2, BIN);
    Serial1.print("E_ERR: ");
    Serial1.println((status2 >> E_ERR) & 0x1, BIN); 
    Serial1.print("P_ERR: ");
    Serial1.println((status2 >> P_ERR) & 0x1, BIN);
    for(int j = 0; j < i; j++){
      Serial1.print(".");
    }
    Serial1.println("");
...
void setup() {
  // Begin the Serial1 Port for Debugging
  Serial1.begin(9600);
  ...
  //Test the interface by reading the Chip ID - report over Serial1
  /*
  ...
  */
  ...
  digitalWrite(cs, LOW);
  SPI.transfer(RUID);
  //Four Dummy Bytes before Serial1 Number Readout
  for(int i = 0; i <= 4; i++){
    SPI.transfer(0x00);
  }  
  uint64_t device_uid = ((uint64_t)SPI.transfer(0x00) << 24) | ((uint64_t)SPI.transfer(0x00) << 16) | (SPI.transfer(0x00) << 8) | SPI.transfer(0x00);
  digitalWrite(cs, HIGH);
  Serial1.print("Manufacturer ID: ");
  Serial1.println(manufacturer_id, HEX);
  Serial1.print("Device ID: ");
  Serial1.println(device_id, HEX);
  Serial1.print("Unique Device ID: ");
  Serial1.println(device_uid, HEX);
  if (manufacturer_id == 0x01){
    digitalWrite(LED_BUILTIN, HIGH);
    if (device_id == 0x6018){
      Serial1.println("S25FL - 128Mb");
    }
    else if (device_id == 0x6019){
      Serial1.println("S25FL - 256Mb");
    }
    else{
    ...
    Serial1.println("Error: Incorrect Manufacturer ID? Please Power Cycle.");
    //for(;;){}
  }
}
...
void loop() {
  ...
  Serial1.print("\n-------- Cycle #");
  Serial1.print(cycle_count);
  Serial1.println(" --------");
  Serial1.print("\nRead/Write Errors Seen: ");
  Serial1.println(xchange_error);

  //STAGE 1: ERASE
  //Enable writes then erase The entire chip
  Serial1.println("ERASE Command Check");
  digitalWrite(cs, LOW);
  ...
  Serial1.println("Command Complete");
  Serial1.println("------");
  ...
  */
  ...
  Serial1.println("WRITE Command Check");
  digitalWrite(cs, LOW);  
  SPI.transfer(WREN); //Write Enable (must be enabled before every command)
  digitalWrite(cs, HIGH);
  digitalWrite(cs, LOW); 
  ...
  Serial1.println("READ Command Check");
  digitalWrite(cs, LOW);
  SPI.transfer(READ);
  SPI.transfer(0x00);
  SPI.transfer(0x00);
  ...
  digitalWrite(cs, HIGH);
  read_status_until_done();
  Serial1.print("Found Data: ");
  Serial1.print(read_data, HEX);
  Serial1.print(" @ ");
  Serial1.println(address, HEX);
  if (read_data != remember_me){
    Serial1.println("ERROR!!! Written and read bytes do not match");
    xchange_error++;
  }
  Serial1.println("Command Complete");
  Serial1.println("------");
